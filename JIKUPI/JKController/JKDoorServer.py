...
class JKDoorServer():
    def __init__(self,statcom_door,hangerstate,logger):
        '''
    ...
    def get_connet_state(self):#获取机库连接状态
        ...
        self.logger.get_log().info(f"连接状态---：{result1}")
        if result1=="9000":
            return "9000"
        else:
    ...
    def oper_door(self,commond):
        ...
        try:
            ...
            if len(commond) != 6 and len(commond) != 2:
                self.logger.get_log().error(f"接收到端命令{commond}，长度不为6")
                return 'error'
            ...
            if commond.startswith("14") or commond.startswith("15"):#门的开启操作
                ...
                if result == "90119021":
                    if commond.startswith("14"):
                        ...
                        # time.sleep(10)
                        #result="9140"
                        # self.hangerstate.set_hanger_door("open")
                    else:
                        result="9151"
                        # time.sleep(10)
                        #result="9150"
                        # self.hangerstate.set_hanger_door("close")
                time.sleep(0.5)
                #2023-5-8 做灯的判断
                if self.config.get_alarm()==True:
                    if result.endswith('1'):
                        alarm=AlarmController(self.hangerstate,self.logger,self.config)
    ...
    def operator_hanger(self,commond):
        ...
        try:
            # 直接调用串口发送命令
            if len(commond) != 6 and len(commond) != 10 and len(commond) != 8 and len(commond) != 12 and len(commond) != 2:
...
if __name__=="__main__":
    ...
    jkcontroller = JKDoorServer(statCom_l, hangstate,logger)
