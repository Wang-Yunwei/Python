...
class GPSInfo():
    ...
    def __init__(self,state,log,comconfig):
        self.state = state
        self.logger = log
    ...
    def start_get_gps(self):
        #启动获取GPS信息的线程
        statCom_gps = JKSATACOM(self.state, self.comconfig.get_device_info_gps(), self.comconfig.get_bps_gps(), self.comconfig.get_timeout_gps(),
                                self.logger,None)
    ...
    def start_get_gps_RTK(self):
        # 启动获取GPS信息的线程
        statCom_gps = JKSATACOM(self.state, self.comconfig.get_device_info_gps(), self.comconfig.get_bps_gps(),
                                self.comconfig.get_timeout_gps(),
                                self.logger, None)
        gps_longitude = 0.0  # 推送的经度
        gps_latitude = 0.0  # 推送的纬度
        while True:
            try:
                if statCom_gps == None:
                    statCom_gps = JKSATACOM(self.state, self.comconfig.get_device_info_gps(),
                                            self.comconfig.get_bps_gps(), self.comconfig.get_timeout_gps(),
                                            self.logger, None)
                statCom_gps.engine.Open_Engine()  # 打开串口
                time.sleep(1)
                ...
                if len(data_result) > 0:
                    ...
                    if (gps_longitude < 10 or gps_latitude < 1):
                        continue
                    # print(f"gps---经纬度返回值为：{self.hex_to_float(data_long)},{self.hex_to_float(data_latitude)}")
                    self.state.set_GPS(f"{gps_longitude-0.00001},{gps_latitude+0.00001}")
                    # print(self.state.getHangerState())
                    # pass
                    # print(int(windspeed,16)/10,winddir,int(rainnum,16))
                time.sleep(self.wait_time)  # 有此操作，信号获取数据稳定
...
if __name__ == "__main__":
    ...
    wfcstate = WFState()
    hangstate = HangerState(wfcstate,None)
    conini=ConfigIni()
    comconfig=USBDeviceConfig(conini)
    ws = GPSInfo(hangstate,logger,comconfig)
    # #启用一个线程
