...
class AutoChargeControl(object):
    ...
    def isNeedCharge(self):
        ...
        state = self.wf_state.get_state()  # 当前充电状态，close/charging/standby/takeoff/outage
            ...
            try:
                if self.configini.get_charge_version() == "wfc":  # 无线充电
                    ...
                    if self.configini.get_wlc_version() == "V1.0":
                        ...
                        result = WFC.operator_charge(recv_text)
                    elif self.configini.get_wlc_version() == "V4.0":  # V4.0
                        WFC = M300JCCServerV4(self.hangstate, self.wf_state, self.logger, self.configini)
                        result = WFC.operator_charge(recv_text)
