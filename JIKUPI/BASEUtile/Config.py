...
class Config():
    ...
    def setconfiginfo(self, ip, socket, station_id, web_socket_url):
    ...
    def getcommond(self):  # command应该误写成commond，修改害怕有问题，先保持原样。
    ...
    def setcommon(self, opendoor, closedoor, openbar, closebar, openair, closeair, openuav, closeuav):
        if self.logger is not None:
            self.logger.get_log().info(
    ...
    def setDetailConfiginfo(self, charge_version, wfc_version, bar_diff_move, GPS, use_weather, weather_485, rain,
    ...
    def set_minio_config(self, minio_ip, minio_username, minio_password, minio_dir):
        if self.logger is not None:
            self.logger.get_log().info(
