...
class VersionInfoXml:
    ...
    def getPath(self):
        return os.path.abspath(self.xml_path)
...
# 主要函数
def AutoUpdate(server_ip, server_port, module_name, order,local_xml_path,update_xml_path):
    ...
    if add_dict == {} and delete_list == []:
        ...
        return False
    # 下载需要更新的文件
    download_progress(add_dict,server_ip, server_port)
    # 文件覆盖到主目录
    ...
    dest_dir = os.path.join(sys.path[0])
...
def analyze_update_info(local_xml, update_xml, module_name):
    ...
    for module_name in module_names:
        ...
        if float(new_xml.getAttribute(module_name)) <= float(old_xml.getAttribute(module_name)):
            continue
        ...
        for file_name in common_list:
            if float(new_xml_objects_dict[file_name]["Version"]) > float(old_xml_objects_dict[file_name]["Version"]):
...
# 下载需要更新的文件
def download_progress(file_info_dict,server_ip, server_port):
    ...
    try:
        ...
        for each_file in file_info_dict.keys():
            ...
            # file_name = each_file
            local_path = os.path.join(sys.path[0], "TempFolder", each_file)
            file_path = os.sep.join(local_path.split("/")[:-1])
            file_name = each_file
            if len(file_name) > 26:
...
def restart_client():
    ...
    #reboot system
    import subprocess
    subprocess.call(['reboot'])#linux

def updatesoftware():
    # region 获取并处理命令行参数
    module_name = "all_module"
    order = "update"
    ...
    del local_xml
    update_flag = AutoUpdate(server_ip, server_port, module_name, order,local_xml_path,update_xml_path)
    # root.destroy()
    time.sleep(15)
