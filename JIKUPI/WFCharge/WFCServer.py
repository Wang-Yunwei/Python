...
class WFCServer():#定义无线充电服务端
    def __init__(self,wf_state,logger,configini):
        ...
        self.comconfig=USBDeviceConfig(self.config)
        #self.engine = Communication(device_info, bps, timeout,self.logger)  # 串口初始化:
        self.engine=SerialHelper(Port=self.comconfig.get_device_info_charge(), BaudRate=self.comconfig.get_bps_charge(), ByteSize=self.comconfig.get_charge_bytesize_charge(), Parity=self.comconfig.get_charge_parity(), Stopbits=self.comconfig.get_charge_stopbits())
    ...
    def operator_charge(self,commond):
        ...
        try:
            ...
            if commond=="Charge":
                ...
                if result!="" and ("Take Off Success" in result or "Rx_CHARGE_CC" in result or "Success" in result or ("Rx_TAKE_OFF_CHECK" in result and "Take Off Fail" not in result and "BatteryVoltageLow" not in result and "Connect Error" not in result)):
                ...
                if result!="" and ("Off Success" in result or "Success" in result or ("Rx_OFF_CHECK" in result and "Off Fail" not in result and "Connect Error" not in result)):
