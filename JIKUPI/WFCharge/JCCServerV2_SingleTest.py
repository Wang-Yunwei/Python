...
-------------------------------------------------------
    ...
    def __init__(self, state, logger, configini):
        self.hangstate=None
    ...
    def check_charging(self,result):
        ...
        if (int(valueAV10) / 100 <45 or int(valueAV10) / 100 >53 or int(valueBV10) / 100<45 or int(valueBV10) / 100>53) and int(valueAV10) / 100!=0 and int(valueBV10) / 100!=0:
                ...
                if value[2:] == "111":  # 双充电，标记位B位A位
                    ...
                    if int(valueAA10) > 0 or int(valueBA10) > 0:#有可能出现单边充电情况
                        ...
                        if self.state.get_state()=="charging" or self.state.get_state()=="full":
                            ...
                            #充电,满电或高温等待
                            if self.pre_model.pre_vol(int(valueAV10) / 100,0)==100 or self.pre_model.pre_vol(int(valueBV10) / 100,0)==100:#冷却的时候电量预测和满电的时候情况类似，开始的时候没有电量
                                #self.state.set_battery_value("100")
                                self.state.set_state("full or waiting")
    ...
    def Check(self):
        ...
        try:
            result = "false"
            # 发送命令
            self.logger.get_log().info(f"发送命令--Check, current battery value is {self.state.get_battery_value()},state is {self.state.get_state()}")
            self.engine = Communication(self.comconfig.get_device_info_chargeV2(), self.comconfig.get_bps_chargeV2(),
                                        10, self.logger, None)
            self.engine.Open_Engine()
            command_read = "01 04 00 00 00 06 70 08"
            ...
            value = binascii.b2a_hex(result[3:5]).decode('ascii')
            value = self.hex2bin(value)
            self.logger.get_log().info(f"check指令获取到的值，value[3:5] is {value[3:5]}，电压、电流值AV is {int(valueAV10) / 100},BV is {int(valueBV10) / 100}，AA is {int(valueAA10) / 100},BA is {int(valueBA10) / 100}，当前电量为{self.state.get_battery_value()}")
