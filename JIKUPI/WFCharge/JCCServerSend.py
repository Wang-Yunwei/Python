...
class M300JCCServerSender():#定义接触充电服务端
    """
    ...
    """
    def __init__(self,hangstate, state, logger,comconfig):
        self.hangstate=hangstate
        self.state = state  # 充电箱当前状态信息
        self.logger = logger
        self.comconfig = comconfig
        self.engine=SerialUtils(self.comconfig,thresholdValue=2)
        ...
        try:
            ...
            while times<waittime:
                ...
                if self.state.get_state()=="charging":
                    ...
                    if self.state.get_state()=="cool":#2023-06-02修复
                        return "chargeerror"
                    return "success"
            return result
        except Exception as e:
    ...
    def takeoff(self):
        ...
        try:
            ...
            while times < waittime:
                time.sleep(1)  # 等待25秒
                times += 1
                if self.state.get_state() == "takeoff":
    ...
    def Connect(self):
        ...
        try:
            ...
            while times < waittime:
                time.sleep(1)  # 等待25秒
                times += 1
                if self.state.get_state() == "connect":
