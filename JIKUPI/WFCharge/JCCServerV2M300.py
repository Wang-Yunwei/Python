...
class JCCServerV2M300:  # 定义接触充电服务端
    """
    A路开
...
-------------------------------------------------------
    ...
    def Check(self):
        ...
        try:
            ...
            self.logger.get_log().info(f"check指令获取到的值，电压、电流值AV is {int(valueAV10) / 100},BV is {int(valueBV10) / 100}，AA is {int(valueAA10) / 100},BA is {int(valueBA10) / 100}，当前电量为{WFState.get_battery_value()}")
