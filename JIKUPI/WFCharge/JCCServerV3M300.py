...
class JCCServerV3M300:  # 定义接触充电服务端
    """
    A路开
...
-------------------------------------------------------
    ...
    def __init__(self, logger):
        ...
        self.engine = Communication(USBDeviceConfig.get_serial_usb_charge(), USBDeviceConfig.get_serial_bps_charge(),
                                    USBDeviceConfig.get_serial_timeout_charge(),
    ...
    def standby(self):
        ...
        try:
            ...
            # self.engine = Communication(self.comconfig.get_serial_usb_charge(), self.comconfig.get_serial_bps_charge(),
            #                             10, self.logger, None)
            ...
            while waittimes > 0:
                if len(result) == 0:
                    # self.engine = Communication(self.comconfig.get_serial_usb_charge(),
                    #                             self.comconfig.get_serial_bps_charge(),
                    #                             10, self.logger, None)
                    # self.engine.Open_Engine()
                    self.engine.Send_data(bytes.fromhex(command_read))
                    result = self.engine.Read_Size(17)  # 读取一行数据
        ...
        try:
            ...
            if value[2:] == "111":  # 充电情况下
                if int(valueAA10) > 0 or int(valueBA10) > 0:  # 电流有一个大于0，标识在充电
                    self.logger.get_log().info(
                        f"Check deal result，电流值 AA is {int(valueAA10) / 100},BA is {int(valueBA10) / 100}")
                    WFState.set_battery_state("charging")
                    result = self.checkcharging_full_process(int(valueAV10) / 100, int(valueBV10) / 100,
                                                             int(valueAA10) / 100, int(valueBA10) / 100)
                elif int(valueAV10) / 100 > 20 or int(valueBV10) / 100 > 20:  # 如果没有电流了，但是有电压，无人机不在或者充满了
                    self.logger.get_log().info(
                        f"Check deal result AV is {int(valueAV10) / 100},BV is {int(valueBV10) / 100}")
                    if WFState.get_battery_state() == "charging" or WFState.get_battery_state() == "full":
                    ...
                    WFState.set_battery_state("close")  # 无人机不在位
                    result = "无人机没连接"
                ...
                WFState.set_battery_state("unkonwn")
            return result
        except Exception as e:
            self.logger.get_log().info(f"待机命令异常，{e}")
            return 'error'
    ...
    def checkcharging_full_process(self, AV, BV, AA, BA):
        ...
        if self.oldAA > 10 or self.oldBA > 10:  # 如果上一次检测的电流都大于10，则本次即使检测出小电流，仍继续充电
            self.logger.get_log().info(
                f"AA电流{AA}，BA电流{BA}，上一次的电流较大，上一次检测AA电流{self.oldAA}，上一次检测BA电流{self.oldBA} ，本次继续充电,当前在充电")
            WFState.set_battery_state("charging")
            result = "charging"
