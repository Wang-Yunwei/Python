...
class WFCV2Listerner:
    ...
    def __init__(self, logger):
        ...
        # 构建一个充电串口对象，进行数据接收
        self.charge_com = SerialUtils(thresholdValue=24)
        self.value = ""  # 接收到的数据
    ...
    def check_state(self):
        ...
        :return:
        '''
        if "Standby Order Received" in self.value and not (
                "ConnectError" in self.value or "PositionError" in self.value or "ConnectBreak" in self.value or "VrecError" in self.value or "InputCurrentError" in self.value):
            ...
            WFState.set_battery_state("cool")  # 电池需要降温，等待中
            self.charge_com.clear_read_value()  # 清空获取到的数据
            ...
            WFState.set_battery_state("chargeerror")  # 充电操作错误
            self.charge_com.clear_read_value()  # 清空获取到的数据
            ...
            if "Drone:OFF" in self.value:
                ...
                # 判断是否为充满电关闭
                if WFState.get_battery_state() == "charging" and WFState.get_battery_value() == "4":  # 上一个时刻还是充电，这个时刻即将关机状态，则判定为充满关机
                    self.logger.get_log().info(f"充电设置 电量100 ;{self.value}")
                    WFState.set_battery_value("100")
                    WFState.set_battery_state("takeoff")
    ...
    def startAlarm(self):
        ...
        判断是否启用警报设置
        '''
        if Config.get_is_alarm() is True:
            if WFState.get_battery_state() == "charging":
                alarm = AlarmController(None, self.logger)
                alarm.start_green_light_slow()
            elif WFState.get_battery_state() == "close":
                alarm = AlarmController(None, self.logger)
                alarm.start_green_light()
            elif WFState.get_battery_state() == "takeoff":
                alarm = AlarmController(None, self.logger)
