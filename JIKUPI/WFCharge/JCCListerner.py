...
class JCCListerner:
    ...
    def __init__(self,logger):
        ...
        #构建一个充电串口对象，进行数据接收
        self.comconfig=Config
        self.charge_com=SerialUtils(thresholdValue=24)#触点充电，返回值少一些
        self.value="" #接收到的数据
    ...
    def check_state(self):
        ...
        if "ChargeStart" in self.value:
            ...
            WFState.set_battery_value("0") #此时电池状态未知
            self.charge_com.clear_read_value()  # 清空获取到的数据
            ...
            # 判断是否为充满电关闭
            # if WFState.get_state() == "charging" and WFState.get_battery_value() == "4":  # 上一个时刻还是充电，这个时刻已经是关机状态，则判定为充满关机；但是出现开启充电成功，1分钟后报失败的情况，这个时候其实是充电失败的
            ...
            #     WFState.set_battery_value("100")
            # if "Drone:OFF" in self.value:
            #     WFState.set_state("close")
            # elif "Drone:ON" in self.value:
            #     WFState.set_state("takeoff")
            self.charge_com.clear_read_value()  # 清空获取到的数据
